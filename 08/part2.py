...
class Solution():
    ...
    def get_closest_coords_pair_diff_clusters(self, clusters):
        shortest=self.distance_matrix[0][1]
        result=None
        for i in range(len(self.distance_matrix)):
            for j in range(i+1, len(self.distance_matrix)):
                dist = self.distance_matrix[i][j]
                if dist < shortest and self.in_diff_clusters(i, j, clusters):
                    shortest= dist
                    result = (i, j)
        return result

    def in_diff_clusters(self, coords1idx, coords2idx, clusters):
        for c in clusters:
            if coords1idx in c:
                return coords2idx not in c 
    ...
    def count_connected(self, start_node):
        ...
        for n in neigh:
            more_count, more_cluster = self.count_connected(n)
            accum_count, cluster = accum_count + more_count, cluster.union(more_cluster)
        return (accum_count, cluster)
    ...
    def solve(self):
        ...
        for n in range(ITERATIONS):
            ...
            print("", a, self.coords[a].get_coords(), b, self.coords[b].get_coords())
        ...
        clusters = []
        for i in range(len(self.coords)):
            size, cluster = self.count_connected(i)
        ...
        while True:
            try:
                a, b = self.get_closest_coords_pair_diff_clusters(clusters)
                self.connect_boxes(a, b)
            except TypeError:
                break
            print("Calculating circuit sizes...")
        ...
        print(a, self.coords[a], b, self.coords[b])
        print(self.coords[a].x, self.coords[b].x)
        print(self.coords[a].x * self.coords[b].x)
